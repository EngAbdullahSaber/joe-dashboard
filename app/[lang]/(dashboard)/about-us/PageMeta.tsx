...
type PageMetaData = {
  ...
  meta: {
    ...
    ogImage: number | null | any; // This is the image ID
    ogUrl: string;
    ogType: string;
...
const PageMeta = () => {
  ...
  const getData = async () => {
    try {
      const res = await getSpecifiedPageMeta(lang, "about-us");
              ...
              return (
                <div key={field.name} className="my-2">
                  ...
                  {field.type === "keywords" && Array.isArray(value) ? (
                    ...
                    <pre className="text-sm  dark:text-gray-200 bg-gray-100 text-slate-700 dark:bg-slate-800 p-2 rounded overflow-auto">
                    ...
                    <div className="space-y-2 text-sm text-gray-600 dark:text-gray-200">
                      {data?.meta?.ogImage?.url ? (
                        <>
                          <img
                            src={`${baseUrl}${data.meta.ogImage.url}`}
                          ...
                          <p>
                            <strong>{t("ID")}:</strong>{" "}
                            {data.meta.ogImage.id || "-"}
                          </p>
                        </>
                      ) : (
                        <p>{t("No image uploaded")}</p>
                      )}
                    </div>
                  ) : (
